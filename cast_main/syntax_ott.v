...
Definition actx : Set := list (atom * atom).
...
Fixpoint open_exp_wrt_castop_rec (k:nat) (c5:castop) (e_5:exp) {struct e_5}: exp :=
  ...
  | (e_fixpoint A e) => e_fixpoint A (open_exp_wrt_castop_rec k c5 e)
...
Fixpoint close_exp_wrt_castop_rec (k:nat) (c5:var) (e_5:exp) {struct e_5}: exp :=
  ...
  | (e_fixpoint A e) => e_fixpoint A (close_exp_wrt_castop_rec k c5 e)
...
Fixpoint castsubst_exp (c5:castop) (cx5:castvar) (e_5:exp) {struct e_5} : exp :=
  ...
  | (e_fixpoint A e) => e_fixpoint A (castsubst_exp c5 cx5 e)
...
Inductive AmberWF : actx -> Prop :=    (* defn AmberWF *)
 | AWF_nil : 
     AmberWF  nil 
 | AWF_cons : forall (AE:actx) (X Y:typevar),
     AmberWF AE ->
     AmberWF  (cons ( X , Y  )  AE ) .

(* defns AmberWellFormedType *)
Inductive AmberWFT : actx -> typ -> Prop :=    (* defn AmberWFT *)
 | AWFT_nat : forall (AE:actx),
     AmberWF AE ->
     AmberWFT AE t_int
 | AWFT_top : forall (AE:actx),
     AmberWF AE ->
     AmberWFT AE t_top
 | AWFT_varl : forall (AE:actx) (X Y:typevar),
     AmberWF AE ->
      In ( X ,  Y )  AE  ->
     AmberWFT AE (t_var_f X)
 | AWFT_varr : forall (AE:actx) (Y X:typevar),
     AmberWF AE ->
      In ( X ,  Y )  AE  ->
     AmberWFT AE (t_var_f Y)
 | AWFT_arrow : forall (AE:actx) (A1 A2:typ),
     AmberWFT AE A1 ->
     AmberWFT AE A2 ->
     AmberWFT AE (t_arrow A1 A2)
 | AWFT_rec : forall (L:vars) (AE:actx) (A:typ) (Y:typevar),
      ( forall X , X \notin  L  -> AmberWFT  (cons ( X , Y  )  AE )   ( open_typ_wrt_typ A (t_var_f X) )  )  ->
     AmberWFT AE (t_mu A).

(* defns AmberSubtyping *)
Inductive AmberSubtyping : actx -> typ -> typ -> Prop :=    (* defn AmberSubtyping *)
 | ASub_nat : forall (AE:actx),
     AmberWF AE ->
     AmberSubtyping AE t_int t_int
 | ASub_top : forall (AE:actx) (A:typ),
     AmberWF AE ->
     AmberWFT AE A ->
     AmberSubtyping AE A t_top
 | ASub_arrow : forall (AE:actx) (A1 A2 B1 B2:typ),
     AmberSubtyping AE B1 A1 ->
     AmberSubtyping AE A2 B2 ->
     AmberSubtyping AE (t_arrow A1 A2) (t_arrow B1 B2)
 | ASub_var : forall (AE:actx) (X Y:typevar),
     AmberWF AE ->
      In ( X ,  Y )  AE  ->
     AmberSubtyping AE (t_var_f X) (t_var_f Y)
 | ASub_rec : forall (L:vars) (AE:actx) (A B:typ),
 ...
 | ASub_self : forall (AE:actx) (A:typ),
     AmberWF AE ->
     AmberWFT AE (t_mu A) ->
     AmberSubtyping AE (t_mu A) (t_mu A).

(* defns Typing *)
Inductive Typing : tctx -> cctx -> ctx -> exp -> typ -> Prop :=    (* defn Typing *)
 | Typing_int : forall (D:tctx) (E:cctx) (G:ctx) (i:int),
     WFTyE D ->
     WFTmE D G ->
     Typing D E G (e_lit i) t_int
 | Typing_var : forall (D:tctx) (E:cctx) (G:ctx) (x:termvar) (A:typ),
     WFTyE D ->
     WFTmE D G ->
      binds  x A G  ->
     Typing D E G (e_var_f x) A
 | Typing_abs : forall (L:vars) (D:tctx) (E:cctx) (G:ctx) (A1:typ) (e:exp) (A2:typ),
      ( forall x , x \notin  L  -> Typing D E  (cons ( x , A1 )  G )   ( open_exp_wrt_exp e (e_var_f x) )  A2 )  ->
     Typing D E G (e_abs A1 e) (t_arrow A1 A2)
 | Typing_app : forall (D:tctx) (E:cctx) (G:ctx) (e1 e2:exp) (A2 A1:typ),
     Typing D E G e1 (t_arrow A1 A2) ->
     Typing D E G e2 A1 ->
     Typing D E G (e_app e1 e2) A2
 | Typing_fix : forall (L:vars) (D:tctx) (E:cctx) (G:ctx) (A:typ) (e:exp),
      ( forall x , x \notin  L  -> Typing D E  (cons ( x , A )  G )   ( open_exp_wrt_exp e (e_var_f x) )  A )  ->
     Typing D E G (e_fixpoint A e) A
 | Typing_cast : forall (D:tctx) (E:cctx) (G:ctx) (c:castop) (e:exp) (B A:typ),
     Typing D E G e A ->
     TypCast D E A B c ->
     Typing D E G (e_cast c e) B
 | Typing_sub : forall (D:tctx) (E:cctx) (G:ctx) (e:exp) (B A:typ),
     Typing D E G e A ->
...
Inductive SynSub : typ -> typ -> Prop :=    (* defn SynSub *)
 | SSub_refl : forall (A:typ),
     lc_typ A ->
     SynSub A A
 | SSub_unfold : forall (A B:typ),
     SynSub A  (open_typ_wrt_typ  B (t_mu B) )  ->
     SynSub A (t_mu B)
 | SSub_arrowL : forall (A B1 B2:typ),
     lc_typ B2 ->
     SynSub A B1 ->
     SynSub A (t_arrow B1 B2)
 | SSub_arrowR : forall (A B1 B2:typ),
     lc_typ B1 ->
     SynSub A B2 ->
     SynSub A (t_arrow B1 B2).

(* defns BtmUpSyntacticSubTerm *)
Inductive BtmUpSub : typ -> typ -> Prop :=    (* defn BtmUpSub *)
 | BSub_refl : forall (A:typ),
     lc_typ A ->
     BtmUpSub A A
 | BSub_unfold : forall (L:vars) (A B:typ),
      ( forall X , X \notin  L  -> BtmUpSub  (open_typ_wrt_typ  A (t_var_f X) )   ( open_typ_wrt_typ B (t_var_f X) )  )  ->
     BtmUpSub  (open_typ_wrt_typ  A (t_mu B) )  (t_mu B)
 | BSub_arrowL : forall (A B1 B2:typ),
     lc_typ B2 ->
     BtmUpSub A B1 ->
     BtmUpSub A (t_arrow B1 B2)
 | BSub_arrowR : forall (A B1 B2:typ),
     lc_typ B1 ->
     BtmUpSub A B2 ->
     BtmUpSub A (t_arrow B1 B2).

(* defns ACSubtyping *)
Inductive ACSubtyping : actx -> typ -> typ -> Prop :=    (* defn ACSubtyping *)
 | ACSub_top : forall (AE:actx) (A:typ),
     lc_typ A ->
     ACSubtyping AE A t_top
 | ACSub_refl : forall (AE:actx) (A:typ),
     lc_typ A ->
     ACSubtyping AE A A
 | ACSub_trans : forall (AE:actx) (A C B:typ),
     ACSubtyping AE A B ->
     ACSubtyping AE B C ->
     ACSubtyping AE A C
 | ACSub_assump : forall (AE1:actx) (X Y:typevar) (AE2:actx),
     ACSubtyping  ( AE2  ++ (cons ( X , Y  )  AE1 ))  (t_var_f X) (t_var_f Y)
 | ACSub_eq : forall (AE:actx) (A B:typ),
      Tyeq  A   B  ->
     ACSubtyping AE A B
 | ACSub_arrow : forall (AE:actx) (A1 A2 B1 B2:typ),
     ACSubtyping AE B1 A1 ->
     ACSubtyping AE A2 B2 ->
     ACSubtyping AE (t_arrow A1 A2) (t_arrow B1 B2)
 | ACSub_rec : forall (L:vars) (AE:actx) (A B:typ),
      ( forall Y , Y \notin  L  ->  ( forall X , X \notin   L  \u {{ Y }}  -> ACSubtyping  (cons ( X , Y  )  AE )   ( open_typ_wrt_typ A (t_var_f X) )   ( open_typ_wrt_typ B (t_var_f Y) )  )  )  ->
     ACSubtyping AE (t_mu A) (t_mu B).

(* defns ACSubtypingAlg *)
Inductive AACSubtyping : actx -> typ -> typ -> Prop :=    (* defn AACSubtyping *)
 | AACSub_top : forall (AE:actx) (A:typ),
     lc_typ A ->
     AACSubtyping AE A t_top
 | AACSub_refl : forall (AE:actx) (A:typ),
     lc_typ A ->
     AACSubtyping AE A A
 | AACSub_assump : forall (AE1:actx) (X Y:typevar) (AE2:actx),
     AACSubtyping  ( AE2  ++ (cons ( X , Y  )  AE1 ))  (t_var_f X) (t_var_f Y)
 | AACSub_eq : forall (AE:actx) (A B:typ),
      Tyeq  A   B  ->
     AACSubtyping AE A B
 | AACSub_arrow : forall (AE:actx) (A1 A2 B1 B2:typ),
     AACSubtyping AE B1 A1 ->
     AACSubtyping AE A2 B2 ->
     AACSubtyping AE (t_arrow A1 A2) (t_arrow B1 B2)
 | AACSub_rec : forall (L:vars) (AE:actx) (A B:typ),
      ( forall Y , Y \notin  L  ->  ( forall X , X \notin   L  \u {{ Y }}  -> AACSubtyping  (cons ( X , Y  )  AE )   ( open_typ_wrt_typ A (t_var_f X) )   ( open_typ_wrt_typ B (t_var_f Y) )  )  )  ->
     AACSubtyping AE (t_mu A) (t_mu B).

(* defns EquiTyping *)
Inductive EquiTyping : tctx -> ctx -> exp -> typ -> Prop :=    (* defn EquiTyping *)
 | ETyping_int : forall (D:tctx) (G:ctx) (i:int),
     WFTyE D ->
     WFTmE D G ->
     EquiTyping D G (e_lit i) t_int
 | ETyping_var : forall (D:tctx) (G:ctx) (x:termvar) (A:typ),
     WFTyE D ->
     WFTmE D G ->
      binds  x A G  ->
     EquiTyping D G (e_var_f x) A
 | ETyping_abs : forall (L:vars) (D:tctx) (G:ctx) (A1:typ) (e:exp) (A2:typ),
      ( forall x , x \notin  L  -> EquiTyping D  (cons ( x , A1 )  G )   ( open_exp_wrt_exp e (e_var_f x) )  A2 )  ->
     EquiTyping D G (e_abs A1 e) (t_arrow A1 A2)
 | ETyping_app : forall (D:tctx) (G:ctx) (e1 e2:exp) (A2 A1:typ),
     EquiTyping D G e1 (t_arrow A1 A2) ->
     EquiTyping D G e2 A1 ->
     EquiTyping D G (e_app e1 e2) A2
 | ETyping_fix : forall (L:vars) (D:tctx) (G:ctx) (A:typ) (e:exp),
      ( forall x , x \notin  L  -> EquiTyping D  (cons ( x , A )  G )   ( open_exp_wrt_exp e (e_var_f x) )  A )  ->
     EquiTyping D G (e_fixpoint A e) A
 | ETyping_sub : forall (D:tctx) (G:ctx) (e:exp) (B A:typ),
     EquiTyping D G e A ->
     ACSubtyping  nil  A B ->
     EquiTyping D G e B.

(* defns algEq *)
Inductive algEq : cctx -> typ -> typ -> castop -> Prop :=    (* defn algEq *)
 | aec_unfold : forall (E:cctx) (A B:typ) (c:castop),
     algEq E  (open_typ_wrt_typ  A (t_mu A) )  B c ->
     algEq E (t_mu A) B (c_seq (c_unfold (t_mu A)) c)
 | aec_fold : forall (E:cctx) (A B:typ) (c:castop),
     algEq E A  (open_typ_wrt_typ  B (t_mu B) )  c ->
     algEq E A (t_mu B) (c_seq c (c_fold (t_mu B)))
 | aec_assump : forall (E:cctx) (A1 A2:typ) (cx:castvar) (A B:typ),
     lc_typ A1 ->
     lc_typ A2 ->
      In ( cx , ( A ,  B ))  E  ->
     algEq E A1 A2 (c_var_f cx)
 | aec_arrfix : forall (L:vars) (E:cctx) (A1 B1 A2 B2:typ) (c1 c2:castop) (A B:typ) (cx':castvar),
      not (   In ( cx' , ( A ,  B ))  E   )  ->
      ( forall cx , cx \notin  L  -> algEq  (cons ( cx , ( (t_arrow A1 B1) ,  (t_arrow A2 B2) ))  E )  A1 A2  ( open_castop_wrt_castop c1 (c_var_f cx) )  )  ->
      ( forall cx , cx \notin  L  -> algEq  (cons ( cx , ( (t_arrow A1 B1) ,  (t_arrow A2 B2) ))  E )  B1 B2  ( open_castop_wrt_castop c2 (c_var_f cx) )  )  ->
     algEq E (t_arrow A1 B1) (t_arrow A2 B2) (c_fixc  (c_arrow c1 c2) )
 | aec_idvar : forall (E:cctx) (X:typevar),
     algEq E (t_var_f X) (t_var_f X) c_id
 | aec_idint : forall (E:cctx),
     algEq E t_int t_int c_id
 | aec_idtop : forall (E:cctx),
     algEq E t_top t_top c_id.

(* defns eqec *)
Inductive eqec : cctx -> typ -> typ -> castop -> Prop :=    (* defn eqec *)
 | ec_assump : forall (E:cctx) (A B:typ) (cx:castvar),
      In ( cx , ( A ,  B ))  E  ->
     eqec E A B (c_var_f cx)
 | ec_refl : forall (E:cctx) (A:typ),
     lc_typ A ->
     eqec E A A c_id
 | ec_trans : forall (E:cctx) (A C:typ) (c1 c2:castop) (B:typ),
     eqec E A B c1 ->
     eqec E B C c2 ->
     eqec E A C (c_seq c1 c2)
 | ec_unfold : forall (E:cctx) (A:typ),
     lc_typ (t_mu A) ->
     eqec E (t_mu A)  (open_typ_wrt_typ  A (t_mu A) )  (c_unfold (t_mu A))
 | ec_fold : forall (E:cctx) (A:typ),
     lc_typ (t_mu A) ->
     eqec E  (open_typ_wrt_typ  A (t_mu A) )  (t_mu A) (c_fold (t_mu A))
 | ec_arr : forall (E:cctx) (A1 B1 A2 B2:typ) (c1 c2:castop),
     eqec E A1 A2 c1 ->
     eqec E B1 B2 c2 ->
     eqec E (t_arrow A1 B1) (t_arrow A2 B2) (c_arrow c1 c2)
 | ec_arrfix : forall (L:vars) (E:cctx) (A1 B1 A2 B2:typ) (c1 c2:castop),
      ( forall cx , cx \notin  L  -> eqec  (cons ( cx , ( (t_arrow A1 B1) ,  (t_arrow A2 B2) ))  E )  A1 A2  ( open_castop_wrt_castop c1 (c_var_f cx) )  )  ->
      ( forall cx , cx \notin  L  -> eqec  (cons ( cx , ( (t_arrow A1 B1) ,  (t_arrow A2 B2) ))  E )  B1 B2  ( open_castop_wrt_castop c2 (c_var_f cx) )  )  ->
     eqec E (t_arrow A1 B1) (t_arrow A2 B2) (c_fixc  (c_arrow c1 c2) ).

(* defns EquiTypingC *)
Inductive EquiTypingC : tctx -> ctx -> exp -> typ -> exp -> Prop :=    (* defn EquiTypingC *)
 | ECTyping_int : forall (D:tctx) (G:ctx) (i:int),
     WFTyE D ->
     WFTmE D G ->
     EquiTypingC D G (e_lit i) t_int (e_lit i)
 | ECTyping_var : forall (D:tctx) (G:ctx) (x:termvar) (A:typ),
     WFTyE D ->
     WFTmE D G ->
      binds  x A G  ->
     EquiTypingC D G (e_var_f x) A (e_var_f x)
 | ECTyping_abs : forall (L:vars) (D:tctx) (G:ctx) (A1:typ) (e:exp) (A2:typ) (e':exp),
      ( forall x , x \notin  L  -> EquiTypingC D  (cons ( x , A1 )  G )   ( open_exp_wrt_exp e (e_var_f x) )  A2  ( open_exp_wrt_exp e' (e_var_f x) )  )  ->
     EquiTypingC D G (e_abs A1 e) (t_arrow A1 A2) (e_abs A1 e')
 | ECTyping_app : forall (D:tctx) (G:ctx) (e1 e2:exp) (A2:typ) (e1' e2':exp) (A1:typ),
     EquiTypingC D G e1 (t_arrow A1 A2) e1' ->
     EquiTypingC D G e2 A1 e2' ->
     EquiTypingC D G (e_app e1 e2) A2 (e_app e1' e2')
 | ECTyping_fix : forall (L:vars) (D:tctx) (G:ctx) (A:typ) (e e':exp),
      ( forall x , x \notin  L  -> EquiTypingC D  (cons ( x , A )  G )   ( open_exp_wrt_exp e (e_var_f x) )  A  ( open_exp_wrt_exp e' (e_var_f x) )  )  ->
     EquiTypingC D G (e_fixpoint A e) A (e_fixpoint A e')
 | ECTyping_eq : forall (D:tctx) (G:ctx) (e:exp) (B:typ) (c:castop) (e':exp) (A:typ),
     EquiTypingC D G e A e' ->
     eqec  nil  A B c ->
     EquiTypingC D G e B (e_cast c e')
 | ECTyping_sub : forall (D:tctx) (G:ctx) (e:exp) (B:typ) (e':exp) (A:typ),
     EquiTypingC D G e A e' ->
     AmberSubtyping  nil  A B ->
     EquiTypingC D G e B e'.


(** infrastructure *)
#[export] Hint Constructors WFT WFTyE ordinary TypCast WFTmE AmberWF AmberWFT AmberSubtyping Typing value Reduction SynSub BtmUpSub ACSubtyping AACSubtyping EquiTyping algEq eqec EquiTypingC lc_typ lc_castop lc_exp : core.
